...
class PathCon(nn.Module):
    ...
    def forward(self, batch):   
        # 原来是重写了nn.Module里的forward方法，
        # 下面train_step中model(batch)这句隐含调用了forward
        if self.use_context:
    ...
    def _get_neighbor_aggregators(self):
        ...
        if self.context_hops == 1:
            ...
            aggregators.append(self.neighbor_agg(batch_size=self.batch_size,
                                                 ...
                                                 act=F.relu))
            # middle layers
            # 除去首层和尾层，中间可以有很多隐藏层，所以range总数-2
    ...
    def _aggregate_neighbors(self, edge_list, mask_list):
        # _aggregate_neighbors 方法的作用是聚合邻居节点的特征，以更新实体的表示。
        # 该方法首先将边缘ID转换为关系特征，然后通过一系列聚合操作，逐步更新实体的特征。
        # 聚合操作考虑了邻居特征和掩码，以确保只聚合相关的邻居信息。
        # 最终，该方法返回聚合后的特征，这些特征将被用于模型的进一步计算。
        # 通过这个方法，模型可以灵活地处理不同跳数的邻居信息，并更新实体的表示。
    ...
    def _rnn(self, path_ids):
        # 首先将路径 ID 转换为路径特征，然后通过 RNN 层处理这些特征，最后通过线性层输出最终结果。
